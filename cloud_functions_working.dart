...
class CloudFunctionsApp extends StatelessWidget {
  ...
  @override
  DeveloperModeWarning build(BuildContext context) {
    return MaterialApp(
...
class _CloudFunctionsDemoState extends State<CloudFunctionsDemo> {
  ...
  void _checkFirebaseConnection() {
    ...
    _addToLogs('Firebase connection simulated (demo mode)');
  }
    ...
    try {
      ...
      final result = {
        'profile': {
          ...
          'displayName': _nameController.text.trim().isEmpty ? 'Demo' : _nameController.text.trim().split(' ')[0],
          'initials': _nameController.text.trim().isEmpty ? 'DU' : _nameController.text.trim().split(' ').map((n) => n.isNotEmpty ? n[0] : '').join('').toUpperCase(),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            // Connection Status
            Card(
              color: _isFirebaseConnected ? Colors.green.shade50 : Colors.red.shade50,
              child: Padding(
                padding: const EdgeInsets.all(16),
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    Text(
                      _isFirebaseConnected ? 'Firebase Connected (Demo Mode)' : 'Firebase Not Connected',
                      style: TextStyle(
                        fontWeight: FontWeight.bold,
                        color: _isFirebaseConnected ? Colors.green : Colors.red,
                      ),
